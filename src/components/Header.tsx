...
import avatarImage from '@/images/cgicon.svg'
  ...
  return (
    <nav {...props}>
      <ul className="flex rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
        ...
        <NavItem href="/speaking">Travels</NavItem>
      </ul>
    </nav>
  )
}
  ...
  return (
    ...
    >
      <Image
        ...
        className={clsx(
          'object-over rounded-full bg-zinc-100 p-0.5 dark:bg-zinc-700',
  ...
  useEffect(() => {
    ...
    function updateAvatarStyles() {
      ...
      setProperty(
        ...
        `tranzinc3d(${x}rem, 0, 0) scale(${scale})`,
      )

      let borderScale = 1 / (toScale / scale)
      let borderX = (-toX + x) * borderScale
      let borderTransform = `tranzinc3d(${borderX}rem, 0, 0) scale(${borderScale})`
